      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white border border-slate-200 rounded-lg p-4 space-y-3 shadow-xs">
          <div className="flex items-center justify-between border-b border-slate-200 pb-2.5">
            <div className="flex items-center gap-2">
              <Presentation className="w-4 h-4 text-teal-700" />
              <h3 className="text-xs font-bold text-slate-800">
                Demo Day 5 頁簡報大綱 (AI 生成)
              </h3>
            </div>
            <button
              onClick={handleGenerateAI}
              disabled={isLoading}
              className="px-2.5 py-1 bg-teal-50 text-teal-800 border border-teal-200 hover:bg-teal-100 rounded text-xs font-semibold transition-all flex items-center gap-1"
            >
          ...
          {slides.length > 0 && (
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 min-h-[200px] flex flex-col justify-between space-y-3">
              <div>
        ...
        <div className="bg-white border border-slate-200 rounded-lg p-4 space-y-3 shadow-xs">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 min-h-[200px] text-xs space-y-3">
            <div className="font-bold text-slate-900 text-sm flex items-center gap-2">
              <Layers className="w-4 h-4 text-teal-700" />
              {mindmap.name}
            </div>

            <div className="pl-4 space-y-2 border-l border-slate-300">
                ...
                <div key={idx} className="space-y-1">
                  ...
                  <div className="pl-4 space-y-1 border-l border-slate-200">
                    {child.children?.map((leaf, lIdx) => (
                      <div key={lIdx} className="text-slate-600 text-[11px]">
                        • {leaf.name}
                      </div>
                    ))}
                  </div>
